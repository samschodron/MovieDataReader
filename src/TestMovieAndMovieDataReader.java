...
public class TestMovieAndMovieDataReader {
	...
	/**
	 * This test reads in 3 movies and checks if the getGenres() method
	 * returns the apprioate lists of genres
	 * @return true if tests pass
	 */
	public boolean testMovieGenres() {
		List<MovieInterface> movieList;
		try {
			movieList = readerToTest.readDataSet(new StringReader(
					"title,original_title,year,genre,duration,country,language,director,writer,production_company,actors,description,avg_vote\n"
		...
		List<String> genre1 = Arrays.asList("Horror");
		List<String> genre2 = Arrays.asList("Action");
		List<String> genre3 = Arrays.asList("Comedy", "Musical", "Romance");

		// check if first movie has the correct list of genres
		if(!movieList.get(0).getGenres().equals(genre1))
			return false;
		// check if second movie has the correct list of genres
		if(!movieList.get(1).getGenres().equals(genre2))
			return false;
		// check if third movie has the correct list of genres
		if(!movieList.get(2).getGenres().equals(genre3))
			return false;
		// true if the three movies have the correct list of genres

		return true;
	}

	/**
	 * This test reads in 3 movies and checks if the getYear() method
	 * returns the apprioate year
	 * @return true if tests pass
	 */
	public boolean testMovieYear() {
		...
		try {
			movieList = readerToTest.readDataSet(new StringReader(
					"title,original_title,year,genre,duration,country,language,director,writer,production_company,actors,description,avg_vote\n"
							+ "The Source of Shadows,The Source of Shadows,2020,Horror,83,USA,English,\"Ryan Bury, Jennifer Bonior\",\"Jennifer Bonior, Trevor Botkin\",Four Thieves Productions,\"Ashleigh Allard, Tom Bonington, Eliane Gagnon, Marissa Kaye Grinestaff, Jenna Heffernan, Joshua Hummel, Janice Kingsley, Chris Labasbas, Jared Laufree, Dominic Lee, Vic May, Sienna Mazzone, Lizzie Mounter, Grace Mumm, Ashley Otis\",\"A series of stories woven together by one of our most primal fears, the fear of the unknown.\",3.5\n"
							+ "The Insurrection,The Insurrection,2018,Action,90,USA,English,Rene Perez,Rene Perez,,\"Michael Par�, Wilma Elles, Joseph Camilleri, Rebecca Tarabocchia, Jeanine Harrington, Malorie Glavan, Danner Boyd, Michael Cendejas, Woody Clendenen, Keely Dervin, Aaron Harvey, Tony Jackson, Michael Jarrod, Angelina Karo, Bernie Kelly\",The director of the largest media company wants to expose how left-wing powers use film to control populations.,2.9\n"
							+ "Valley Girl,Valley Girl,2016,\"Comedy, Musical, Romance\",102,USA,English,Rachel Lee Goldenberg,\"Amy Talkington, Andrew Lane\",Sneak Preview Productions,\"Jessica Rothe, Josh Whitehouse, Jessie Ennis, Ashleigh Murray, Chloe Bennet, Logan Paul, Mae Whitman, Mario Revolori, Rob Huebel, Judy Greer, Alex Lewis, Alex MacNicoll, Danny Ramirez, Andrew Kai, Allyn Rachel\",\"Set to a new wave '80s soundtrack, a pair of young lovers from different backgrounds defy their parents and friends to stay together. A musical adaptation of the 1983 film.\",5.4\n"
			));
		} catch (Exception e) {
			e.printStackTrace();
			// test failed
		...
		Integer year3 = 2016;
		boolean equalOne = true;
		// check if first movie has the correct year
		...
		// check if second movie has the correct year
		...
		// check if third movie has the correct year
